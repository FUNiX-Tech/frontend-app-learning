...
function FunixLearningGoalCard({
  ...
  const filterCourseBlock = courseDateBlocks.filter(
    (courseBlock) => courseBlock.title !== "Enrollment Date"
  );
  ...
  const [selectBlockId, SetSelectBlockId] = useState(
    filterCourseBlock[0]?.link?.split("/")[6] || ""
  );
      ...
      <Card.Body className=" p-3 p-lg-3.5">
        <div className="row " style={{ gap: "30px" }}>
          <div className="col-2.5 form-date ">
            <h2 className="h4 mb-1 text-primary-500">
              {intl.formatMessage(messages.setHourDailyText)}
            </h2>
          ...
          <div className="col-2.5 form-date">
            <h2 className="h4 mb-2.5 text-primary-500 ">
              Nhập ngày bắt đầu học
            </h2>
            <Input
              type="date"
              value={selectedDate}
          ...
          <div className="col form-date">
            <h2 className="h4 mb-2.5 text-primary-500 ">
              Chọn bài bắt đầu học
            </h2>
            <div>
              <select
                className="form-control w-100"
                value={selectBlockId}
                onChange={(e) => SetSelectBlockId(e.target.value)}
              >
                {filterCourseBlock.map((courseBlock) => {
                  const blockId = courseBlock.link.split("/")[6];
                  return (
                    <option value={blockId} key={blockId}>
                      {courseBlock.title}
                    </option>
                  );
                })}
          ...
          <div className="col form-date " style={{ marginLeft: "-18px" }}>
            <h2 className="h4 mb-1 text-primary-500">
              {intl.formatMessage(messages.setWeekdayText)}
            </h2>
            <div className="text-gray-700 small mb-2.5">
                ...
                return (
                  <button
                    ...
                    className={classnames(
                      "flag-button row w-100 align-content-between m-1.5",
                      isSelected ? "funix-flag-button-selected" : ""
                    )}
                    onClick={() => handleSelect(index)}
                  >
                    <div
                      className={classnames(
                        "row w-100 m-0 justify-content-center small text-gray-700 pb-1 pt-1",
        ...
        <div className="d-flex justify-content-between align-items-end pt-3">
          <div className=" ">
            <span style={{ fontWeight: "650" }}>
              Kế hoạch học tập dự kiến dựa trên quỹ tời gian cá nhân của học
              viên
            </span>
          </div>
          <div className="">
