...
const SearchCourse = () => {
  ...
  const handlerSearch = async () => {
    try {
      const page_index = 0;
      const { data, total } = await fetchSearchCourse(
        courseIdFromUrl,
        inputValue,
        page_index
      );
    ...
    <div className="search-course-custom">
      ...
      <ModalLayer isOpen={isOpen} onClose={close}>
        <div
          ...
          className="modal-search mw-sm p-4 bg-white mx-auto my-5 rounded "
        >
          <div className="modal-header-search d-flex justify-content-between align-items-center">
            <span className="search-title">Search</span>
            <span onClick={close} className="search-close rounded">
          ...
          <span className="search-title" style={{ fontSize: "1.2rem" }}>
            Results :
          </span>
          <div className="search-results">
            {resultSearch.length > 0 ? (
              resultSearch.map((e) => {
                return (
                  <div className="p-2" onClick={() => handlerNavigate(e)}>
                    <div className="result-item rounded border p-4">
                      ...
                      <div
                        className="excerpt"
                        dangerouslySetInnerHTML={{ __html: e.data.excerpt }}
                      />
                    </div>
                  </div>
                );
            ...
            {resultSearch.length > 0 && (
              <div className="text-center">
                <button className="btn-load-more" onClick={handlerLoadMore}>
                  Load More
                </button>
              </div>
            )}
