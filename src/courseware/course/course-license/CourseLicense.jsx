...
function CourseLicense({ license, intl }) {
  const renderAllRightsReservedLicense = () => (
    <div className="text-gray-500">
      <FontAwesomeIcon aria-hidden="true" className="mr-1" icon={faCopyright} />
      {intl.formatMessage(
        messages["learn.course.license.allRightsReserved.text"]
      )}
    </div>
  );

  const renderCreativeCommonsLicense = (
    activeCreativeCommonsLicenseTags,
    version
  ) => (
    <a
      ...
      target="_blank"
      href={`https://creativecommons.org/licenses/${activeCreativeCommonsLicenseTags.join(
        "-"
      )}/${version}/`}
    >
      <span className="sr-only">
        {intl.formatMessage(
          messages["learn.course.license.creativeCommons.terms.preamble"]
        )}
        &nbsp;
      </span>
      <FontAwesomeIcon
      ...
      {activeCreativeCommonsLicenseTags.map((tag) => (
        <span key={tag}>
          <span className="sr-only">
            {intl.formatMessage(
              messages[CreativeCommonsLicenseTags[tag].intlMessagesId]
            )}
            &nbsp;
          </span>
          <FontAwesomeIcon
      ...
      {intl.formatMessage(
        messages["learn.course.license.creativeCommons.text"]
      )}
    </a>
  );
